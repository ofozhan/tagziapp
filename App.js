...
// BİLDİRİM AYARI: Uygulama açıkken bildirim geldiğinde nasıl davranacağını belirler.
...
function TodayScreen() {
  ...
  const handleEndDay = () => {
    Alert.alert( "Günü Bitir", "Bugünkü veriler kaydedilecek ve ekran yeni bir gün için sıfırlanacak. Emin misiniz?",
      [ { text: "İptal", style: "cancel" },
        { text: "Evet, Bitir", onPress: () => {
            setStartKm(''); setEndKm(''); setKazanclar([]); setEkstraMasraflar([]);
            setInputTutar(''); setInputNot('');
            setToday(getTodayDateString());
            Alert.alert("Gün Bitti", "Verileriniz kaydedildi. Ekran yeni gün için hazır!");
          }, style: "destructive",
        },
      ]
    );
  ...
  const handleKayitEkle = (tip) => {
    const tutar = parseFloat(inputTutar);
    if (!tutar || tutar <= 0) { Alert.alert('Hata', 'Lütfen geçerli bir tutar girin.'); return; }
    const yeniKayit = { id: Date.now().toString(), tutar, not: inputNot };
    if (tip === 'kazanc') { setKazanclar(onceki => [yeniKayit, ...onceki]); } 
    else { setEkstraMasraflar(onceki => [yeniKayit, ...onceki]); }
    setInputTutar(''); setInputNot(''); Keyboard.dismiss();
  };

  const gunlukOzet = useMemo(() => {
    const baslangic = parseFloat(startKm) || 0;
    const bitis = parseFloat(endKm) || 0;
    const maliyetPerKm = parseFloat(yakitMaliyeti.replace(',', '.')) || 0;
    const toplamKm = bitis > baslangic ? bitis - baslangic : 0;
    const yakitMasrafi = toplamKm * maliyetPerKm;
    const toplamKazanc = kazanclar.reduce((acc, val) => acc + val.tutar, 0);
    const toplamEkstraMasraf = ekstraMasraflar.reduce((acc, val) => acc + val.tutar, 0);
    const toplamMasraf = yakitMasrafi + toplamEkstraMasraf;
    const netKar = toplamKazanc - toplamMasraf;
    return { toplamKm, yakitMasrafi, toplamKazanc, toplamEkstraMasraf, toplamMasraf, netKar };
  }, [startKm, endKm, yakitMaliyeti, kazanclar, ekstraMasraflar]);

  const renderKayit = ({ item, tip }) => (
    <View style={styles.kayitItem}>
      <Text style={[styles.kayitTutar, { color: tip === 'kazanc' ? '#2ecc71' : '#e74c3c' }]}>
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <>
              ...
              <View style={styles.listelerContainer}><ListeKolonu baslik="Kazançlar" data={kazanclar} renderItem={({ item }) => renderKayit({ item, tip: 'kazanc' })} /><ListeKolonu baslik="Ekstra Masraflar" data={ekstraMasraflar} renderItem={({ item }) => renderKayit({ item, tip: 'masraf' })} /></View>
  ...
  const renderHistoryItem = ({ item }) => (
    ...
    >
      ...
      <View>
        ...
        <Text style={[styles.netProfitText, { color: item.netKar >= 0 ? '#2ecc71' : '#e74c3c' }]}>Net Kâr: {formatCurrency(item.netKar)}</Text>
      </View>
    </TouchableOpacity>
  );
...
//================================================================
// DAY DETAIL SCREEN BÖLÜMÜ
//================================================================

const KayitItem_detail = ({ item, tip, onEdit }) => (
    <View style={styles.kayitItem_detail}>
        <View style={{ flex: 1 }}>
            <Text style={[styles.kayitTutar_detail, { color: tip === 'kazanc' ? '#2ecc71' : '#e74c3c' }]}>
                {tip === 'kazanc' ? '+ ' : '- '}{formatCurrency(item.tutar)}
            </Text>
        ...
        <TouchableOpacity onPress={onEdit} style={styles.editButton_detail}>
            <Ionicons name="pencil-outline" size={20} color="#bdc3c7" />
        </TouchableOpacity>
    </View>
);
...
function DayDetailScreen({ route, navigation }) {
  ...
  const handleDeleteDay = useCallback(() => {
    Alert.alert( "Kaydı Sil", `${new Date(date).toLocaleDateString('tr-TR')} tarihli kaydı kalıcı olarak silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.`,
      [ { text: "İptal", style: "cancel" },
        { text: "Evet, Sil", onPress: async () => {
            try {
              const storageKey = `@TagziApp:data_${date}`;
              await AsyncStorage.removeItem(storageKey);
              navigation.goBack();
            } catch (e) { console.error("Failed to delete data", e); Alert.alert("Hata", "Kayıt silinirken bir sorun oluştu."); }
          }, style: "destructive",
        },
      ]
    );
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerRight: () => (
        <TouchableOpacity onPress={handleDeleteDay} style={{ marginRight: 15 }}>
          <Ionicons name="trash-outline" size={24} color="#e74c3c" />
        </TouchableOpacity>
      ),
    });
  }, [navigation, handleDeleteDay]);
  ...
  const loadDayData = useCallback(async () => {
      ...
      try {
        ...
        if (dataString) {
          ...
          const toplamKazanc = (parsedData.kazanclar || []).reduce((sum, item) => sum + item.tutar, 0);
          const baslangic = parseFloat(parsedData.startKm) || 0;
          const bitis = parseFloat(parsedData.endKm) || 0;
          const maliyetPerKm = parseFloat(parsedData.yakitMaliyeti) || 0;
          const toplamKm = bitis > baslangic ? bitis - baslangic : 0;
          const yakitMasrafi = toplamKm * maliyetPerKm;
          const toplamEkstraMasraf = (parsedData.ekstraMasraflar || []).reduce((sum, item) => sum + item.tutar, 0);
          const toplamMasraf = yakitMasrafi + toplamEkstraMasraf;
          const netKar = toplamKazanc - toplamMasraf;
          setDayData({ ...parsedData, toplamKazanc, toplamMasraf, toplamKm, netKar });
        }
      } catch (e) { console.error("Failed to load day data", e); } 
      finally { setIsLoading(false); }
    }, [date]);

  useEffect(() => { loadDayData(); }, [loadDayData]);

  const openEditModal = useCallback((item, tip) => {
    setEditingItem({ ...item, tip });
    setEditTutar(item.tutar.toString());
    setEditNot(item.not || '');
    setIsModalVisible(true);
  }, []);

  const handleSaveChanges = useCallback(async () => {
    if (!editingItem || !editTutar) { Alert.alert("Hata", "Tutar boş bırakılamaz."); return; }
    const updatedDayData = JSON.parse(JSON.stringify(dayData));
    const listToUpdate = editingItem.tip === 'kazanc' ? updatedDayData.kazanclar : updatedDayData.ekstraMasraflar;
    const itemIndex = listToUpdate.findIndex(item => item.id === editingItem.id);
    if (itemIndex > -1) {
      listToUpdate[itemIndex].tutar = parseFloat(editTutar.replace(',', '.')) || 0;
      listToUpdate[itemIndex].not = editNot;
    }
    try {
      const storageKey = `@TagziApp:data_${date}`;
      await AsyncStorage.setItem(storageKey, JSON.stringify(updatedDayData));
      await loadDayData();
      setIsModalVisible(false); setEditingItem(null);
    } catch (e) { console.error("Failed to save edited data", e); Alert.alert("Hata", "Değişiklikler kaydedilirken bir sorun oluştu."); }
  }, [editingItem, editTutar, editNot, dayData, date, loadDayData]);
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView>
        <View style={styles.ozetKarti_detail}>
          <OzetSatiri_detail label="Net Kâr:" deger={formatCurrency(dayData.netKar)} renk={dayData.netKar >= 0 ? '#2ecc71' : '#e74c3c'}/>
        ...
        <View style={styles.listelerContainer_detail}>
          <View style={{ flex: 1 }}>
            <Text style={styles.listeBaslik_detail}>Kazançlar</Text>
            {dayData.kazanclar && dayData.kazanclar.length > 0 ? ( dayData.kazanclar.map(item => <KayitItem_detail key={item.id} item={item} tip="kazanc" onEdit={() => openEditModal(item, 'kazanc')}/>)) : ( <Text style={styles.emptyListText_detail}>Kazanç kaydı yok.</Text> )}
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.listeBaslik_detail}>Ekstra Masraflar</Text>
            {dayData.ekstraMasraflar && dayData.ekstraMasraflar.length > 0 ? ( dayData.ekstraMasraflar.map(item => <KayitItem_detail key={item.id} item={item} tip="masraf" onEdit={() => openEditModal(item, 'masraf')}/>)) : ( <Text style={styles.emptyListText_detail}>Masraf kaydı yok.</Text> )}
          </View>
        </View>
      </ScrollView>
      <Modal animationType="fade" transparent={true} visible={isModalVisible} onRequestClose={() => setIsModalVisible(false)}>
        <View style={styles.modalContainer_detail}>
...
//================================================================
// ANA APP COMPONENT'İ VE NAVIGASYON
//================================================================
...
export default function App() {
  const [fontsLoaded] = useFonts({ Poppins_400Regular, Poppins_700Bold, Antonio_700Bold });
  ...
  useEffect(() => {
    const setupNotifications = async () => {
      ...
      if (finalStatus !== 'granted') {
        console.log('Bildirim gönderme izni verilmedi!');
        return;
      }
      ...
      await Notifications.scheduleNotificationAsync({
        content: {
          title: "Günü kapatma zamanı! 잊",
          body: 'Bugünkü kayıtlarını Tagzi\'ye eklemeyi unutma.',
          sound: 'default',
        },
      ...
      <NavigationContainer>
        <Tab.Navigator
          screenOptions={({ route }) => ({
            ...
            headerTitleStyle: { color: '#fff', fontFamily: 'Antonio_700Bold', fontSize: 32 },
            tabBarLabelStyle: { fontFamily: 'Poppins_700Bold' }
          })}
        >
...
const styles = StyleSheet.create({
  ...
  ozetLabel: { color: '#bdc3c7', fontSize: 16, fontFamily: 'Poppins_400Regular' },
  netKarText: { color: '#2ecc71', fontSize: 36, fontFamily: 'Poppins_700Bold', marginBottom: 10 },
  ozetSatiri: { flexDirection: 'row', justifyContent: 'space-between', marginVertical: 4 },
  ozetDeger: { fontSize: 16, fontFamily: 'Poppins_700Bold' },
  separator: { height: 1, backgroundColor: '#485460', marginVertical: 15 },
  masrafDetay: { fontSize: 12, fontFamily: 'Poppins_400Regular', color: '#95a5a6', textAlign: 'right', marginTop: -4, marginBottom: 8 },
  ...
  formBaslik: { color: 'white', fontSize: 18, fontFamily: 'Poppins_700Bold' },
  collapsibleContent: { marginTop: 20 },
  inputLabel: { color: '#bdc3c7', fontSize: 14, fontFamily: 'Poppins_400Regular', marginBottom: 5 },
  ...
  listeBaslik: { color: 'white', fontSize: 18, fontFamily: 'Poppins_700Bold', marginBottom: 10, textAlign: 'center' },
  kayitItem: { backgroundColor: '#34495e', padding: 10, borderRadius: 5, marginBottom: 5 },
  kayitTutar: { fontFamily: 'Poppins_700Bold', fontSize: 16 },
  notText: { fontFamily: 'Poppins_400Regular', fontSize: 13, color: '#bdc3c7', marginTop: 3 },
  ...
  endDayButtonText: { color: 'white', fontFamily: 'Poppins_700Bold', fontSize: 16, marginLeft: 10 },
  ...
  summaryLabel: { color: '#bdc3c7', fontFamily: 'Poppins_400Regular', fontSize: 12, textAlign: 'center' },
  summaryValue: { color: 'white', fontFamily: 'Poppins_700Bold', fontSize: 16, marginTop: 5 },
  ...
  dateText: { color: 'white', fontFamily: 'Poppins_700Bold', fontSize: 16 },
  earningsText: { color: '#bdc3c7', fontFamily: 'Poppins_400Regular', fontSize: 14, textAlign: 'right' },
  netProfitText: { fontFamily: 'Poppins_700Bold', fontSize: 14, textAlign: 'right' },
  emptyText: { color: '#95a5a6', textAlign: 'center', marginTop: 50, fontFamily: 'Poppins_400Regular' },
  ...
  ozetLabel_detail: { color: '#bdc3c7', fontSize: 16, fontFamily: 'Poppins_400Regular' },
  ozetDeger_detail: { fontSize: 16, fontFamily: 'Poppins_700Bold', color: '#fff' },
  ...
  listeBaslik_detail: { color: 'white', fontSize: 18, fontFamily: 'Poppins_700Bold', marginBottom: 10, textAlign: 'center' },
  ...
  kayitTutar_detail: { fontFamily: 'Poppins_700Bold', fontSize: 16 },
  notText_detail: { fontFamily: 'Poppins_400Regular', fontSize: 13, color: '#bdc3c7', marginTop: 3 },
  dateText_detail: { color: 'white', fontFamily: 'Poppins_700Bold', fontSize: 18, textAlign: 'center' },
  emptyListText_detail: { fontFamily: 'Poppins_400Regular', color: '#95a5a6', textAlign: 'center', fontSize: 14, padding: 10, },
  ...
  modalTitle_detail: { color: 'white', fontSize: 20, fontFamily: 'Poppins_700Bold', marginBottom: 20, textAlign: 'center' },
  ...
  buttonText_detail: { color: 'white', fontFamily: 'Poppins_700Bold', fontSize: 16 },
});